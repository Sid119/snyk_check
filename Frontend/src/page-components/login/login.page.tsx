"use client";
import React, { useContext, useEffect, useState } from "react";
import NavbarLoginComponent from "./navbar-login.part";
import styles from "./login.part.module.scss";
import { Button } from "@/src/components/ui/button";
import { useMutation, useQuery } from "@tanstack/react-query";
import { LoginBody, getSsoUrl } from "@/src/repo/sso";
import { useRouter } from "next/router";
import Cookie from "js-cookie";
import { AppContext } from "@/src/context/app-context";
import { Input } from "@/src/components/ui/input";

const SignIn: React.FC = () => {
    const appContext = useContext(AppContext);
    const router = useRouter();
    const [ssoEmailId, setSsoEmailId] = useState<string>("");
    const [login, setLogin] = useState<string>("login");

    const handleLoginPage = () => {
        setLogin("ssoLogin");
    };

    const {
        error: loginError,
        data: loginData,
        isLoading: loginLoading,
        mutate: loginReq,
    } = useMutation<any, Error, LoginBody>({
        mutationKey: ["getSsoUrl"],
        mutationFn: getSsoUrl,
    });

    const onSubmit = () => {
        loginReq({
            emailId: ssoEmailId,
        });
    };

    useEffect(() => {
        if (loginData) {
            Cookie.set("user", JSON.stringify(loginData.user));
            window.location.href = loginData.url;
        }
    }, [loginData]);

    useEffect(() => {
        if (router.query.code && typeof router.query.code === "string") {
            Cookie.set("code", router.query.code);
            window.location.href = "/";
        }
    }, [router.query.code]);

    useEffect(() => {
        if (appContext?.code) {
            router.push("/");
        }
    }, [appContext?.code, router]);

    if (appContext?.isLoggedIn || router.query.code) {
        return <p>Loading...</p>;
    }

    return (
        <main className="flex flex-col h-screen">
            <NavbarLoginComponent />
            <div className="w-full h-screen bg-[#303C54] relative">
                {login === "login" && (
                    <div className={styles.loginContainer}>
                        <div className={styles.loginText}>
                            Sign in with your email and password
                        </div>
                        <form className={styles.loginFileds}>
                            <label className="text-white pb-1 leading-5 font-medium text-[12px] ">
                                Email
                            </label>
                            <input
                                type="email"
                                className="rounded-sm w-[300px] px-6 py-2 h-10 mb-2"
                                placeholder="name@cloudeq.com"
                                name="email"
                                required
                            />
                            <label className="text-white pb-2 leading-5 font-medium text-[12px]">
                                Password
                            </label>
                            <input
                                type="password"
                                className="rounded-sm px-6 py-2 h-10"
                                placeholder="Password"
                                name="password"
                                required
                            />
                            <input
                                type="submit"
                                value="Sign in"
                                className={styles.signIn}
                            />
                            <div className={styles.line}>Or</div>
                        </form>
                        <div className={styles.signinContainer}>
                            <div className="font-semibold text-[16px] leading-5 text-[fff]">
                                Sign in with your corporate ID
                            </div>
                            <Button
                                className={styles.ssoSign}
                                onClick={handleLoginPage}
                            >
                                SSO-Sign in
                            </Button>
                        </div>
                    </div>
                )}
                {login !== "login" && (
                    <div className={styles.loginContainer}>
                        <div className={styles.loginText}>
                            SSO Sign In
                        </div>
                        <form
                            onSubmit={(e) => {
                                e.preventDefault();
                                onSubmit();
                            }}
                            className={styles.loginFileds}
                        >
                            <label className="text-white pb-1 leading-5 font-medium text-[12px] ">
                                Email
                            </label>
                            <Input
                                type="email"
                                className="rounded-sm w-[300px] px-6 py-2 h-10 mb-2"
                                placeholder="name@cloudeq.com"
                                name="email"
                                value={ssoEmailId}
                                onChange={(e) => {
                                    setSsoEmailId(e.target.value);
                                }}
                                required
                            />
                            <Button type="submit" className={styles.signIn} disabled={loginLoading || loginData}>
                                {loginLoading && "Loading..."}
                                {loginData && "Redirecting..."}
                                {(!loginLoading && !loginData) && "Sign In"}
                            </Button>
                            {loginError && <p className="text-white mt-3">* {loginError.message}</p>}
                        </form>
                    </div>
                )}
            </div>
        </main>
    );
};

export default SignIn;
